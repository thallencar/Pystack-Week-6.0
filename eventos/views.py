...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        fonte_nome = ImageFont.truetype(path_fonte, 70)
        fonte_info = ImageFont.truetype(path_fonte, 40)
        draw.text((230, 630), f"{participante.username}", font=fonte_nome, fill=(0, 0, 0))
        draw.text((761, 770), f"{evento.nome}", font=fonte_info, fill=(0, 0, 0))
        draw.text((816, 840), f"{evento.carga_horaria} horas", font=fonte_info, fill=(0, 0, 0))
        output = BytesIO()
